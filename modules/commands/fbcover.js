...
if ((this.config.credits) != "Amir") { return api.sendMessage(`[ WARNING ] - Itz Amir Don't credits modules ${this.config.name} was changed to ${this.config.credits} bởi ADMINBOT ${global.config.BOTNAME} 😐 Stop now!!!`, event.threadID, event.messageID)}
    const request = require('request');
  const fs = require("fs-extra")
  const axios = require("axios")
  const { threadID, messageID, senderID, body } = event;
  if(!args[0]) return api.sendMessage('Hey Itz Me Amir here Please enter primary name!!!', threadID, messageID)
  else return api.sendMessage(`Itz Amir😍 You chose the main name: ${args.join(" ").toUpperCase()}\n\n(Reply to this message and choose your secondary name)`,event.threadID, (err, info) => {
...
module.exports.handleReply = async function({ api, event, args, handleReply, client, __GLOBAL, Threads, Users, Currencies }) {
  ...
  switch (handleReply.type) {
    case "tenphu": {
      ...
      return api.sendMessage(`Itz Amir🐰 chose the sub-name ${event.body.toUpperCase()}\n\n(Reply to this message enter your phone number)`,threadID, function (err, info) {
        return global.client.handleReply.push({
          type: "sdt",
          name: `fbcover`,
          author: senderID,
    ...
    case "sdt": {
      api.unsendMessage(handleReply.messageID);
      return api.sendMessage(`Amir Here You have selected SDT as: ${event.body.toUpperCase()}\n\(Reply this message to enter your email)`,threadID, function (err, info) {
        return global.client.handleReply.push({
          type: "email",
          name: `fbcover`,
          author: senderID,
    ...
    case "email": {
      api.unsendMessage(handleReply.messageID);
      return api.sendMessage(`Itz Amir😘 You have selected email as : ${event.body.toUpperCase()}\n\(Reply this message to enter your address)`,threadID, function (err, info) {
        return global.client.handleReply.push({
          type: "color",
          name: `fbcover`,
          author: senderID,
    ...
    case "create": {
      ...
      api.unsendMessage(handleReply.messageID);
      api.sendMessage(`Hey Itz Me Amir Your Cover Almost Done Please Wait⏳ Initializing the imager!`,threadID, (err, info) => {
      setTimeout(() => {
